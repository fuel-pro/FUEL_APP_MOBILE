...
export async function GET(): Promise<Response> {
  return new Response(JSON.stringify({
    ...
    features: {
      fileUpload: {
        supported: ['PDF', 'Images (JPEG, PNG, GIF, WebP)', 'Spreadsheets (XLSX, CSV)', 'Text files'],
        maxFileSize: '10MB',
        maxFiles: 10,
      },
      ...
      fuelTypes: ['PMS (Petrol)', 'AGO (Diesel)', 'LPG (Cooking Gas)', 'Kerosene', 'Super (Premium)', 'ULP (Unleaded)'],
      shifts: ['DAY', 'NIGHT', 'MORNING', 'AFTERNOON', 'FULL'],
    },
    
    exportFormats: {
      csv: { name: 'CSV', description: 'Comma-separated values', mimeType: 'text/csv' },
      txt: { name: 'Plain Text', description: 'No formatting, universal compatibility', mimeType: 'text/plain' },
      json: { name: 'JSON', description: 'Machine-readable format', mimeType: 'application/json' },
      html: { name: 'HTML', description: 'Printable document', mimeType: 'text/html' },
      pdf: { name: 'PDF', description: 'Print via HTML', mimeType: 'text/html' },
      xlsx: { name: 'Excel', description: 'Spreadsheet format', mimeType: 'application/vnd.ms-excel' },
    },
