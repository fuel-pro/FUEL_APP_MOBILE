...
-- ============================================================
-- FOUNDER AUDIT LOG TABLE
-- Stores audit events for founder/admin panel actions
-- ============================================================
...
-- ============================================================
-- HELPER FUNCTION: Check if user is founder/admin
-- ============================================================
...
AS $$
  ...
  OR
  -- Also allow users with no profile but with auth ID (for initial setup)
  -- They must have a valid session in auth.users
  EXISTS (
    SELECT 1
    FROM auth.users u
    WHERE u.id = uid
...
-- Only founder/admin users can read the audit log
-- ============================================================
CREATE POLICY "founder_read_audit_log"
ON public.founder_audit_log
FOR SELECT
TO authenticated
USING (
...
-- Anyone with a session can insert (authorized at app level)
CREATE POLICY "founder_insert_audit_log"
ON public.founder_audit_log
FOR INSERT
TO authenticated
WITH CHECK (
...
-- ============================================================
-- FOUNDER SESSIONS TABLE
-- Stores founder session data (2FA, password, contact)
-- ============================================================
...
-- Only founder/admin users can access their session data
-- ============================================================
CREATE POLICY "founder_read_session"
...
CREATE POLICY "founder_insert_session"
ON public.founder_sessions
FOR INSERT
TO authenticated
WITH CHECK (
...
CREATE POLICY "founder_update_session"
ON public.founder_sessions
FOR UPDATE
TO authenticated
USING (
...
-- ============================================================
-- FUNCTION: Write founder audit log entry
-- Security definer to allow inserts from app code
-- ============================================================
...
BEGIN
  ...
  INSERT INTO public.founder_audit_log (
    actor_id,
    action,
    entity_type,
    entity_id,
...
AS $$
DECLARE
  session_record public.founder_sessions;
BEGIN
  -- Verify user is authenticated
  IF auth.uid() IS NULL THEN
    RAISE EXCEPTION 'not authenticated';
  ...
  FROM public.founder_sessions
  LIMIT 1;

  -- If no session exists, create one
  IF session_record IS NULL THEN
    INSERT INTO public.founder_sessions DEFAULT VALUES
...
BEGIN
  ...
  UPDATE public.founder_sessions
  SET
...
-- ============================================================
-- TRIGGER: Update founder_sessions timestamp
-- ============================================================
...
FOR EACH ROW EXECUTE FUNCTION update_founder_sessions_updated_at();

-- ============================================================
-- Add profiles table if it doesn't exist
-- (needed for is_founder check)
-- ============================================================
