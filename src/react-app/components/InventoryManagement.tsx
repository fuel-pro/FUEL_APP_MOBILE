...
import { automation, emit, on, getAutoReorders, fulfillReorder } from "@/react-app/lib/automation-engine";
...
type InventoryTab =
  "products" | "adjustments" | "transfers" | "counts" | "wastage" | "history" | "reorders";
...
const ProductTableHeader = () => (
  <thead>
    <tr className="border-b border-white/10">
      {["Product", "Category", "Stock", "Cost", "Price", "Status", "Actions"].map((label) => (
        <th key={label} className="text-left text-xs font-semibold text-gray-400 px-4 py-3">
          {label}
        </th>
      ))}
    </tr>
  ...
  <tr className="border-b border-white/5 hover:bg-white/5">
    ...
    <td className="px-4 py-4">
      <span
        className={`text-xs px-2 py-1 rounded-full ${
          product.is_active ? "bg-emerald-500/20 text-emerald-400" : "bg-gray-500/20 text-gray-400"
        }`}
      >
    ...
    <td className="px-4 py-4">
      <div className="flex items-center gap-2">
        <button onClick={() => onEdit(product)} className="p-2 text-gray-400 hover:text-white hover:bg-white/10 rounded-lg transition-colors">
          <Edit2 size={16} />
        </button>
        <button onClick={() => onDelete(product)} className="p-2 text-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors">
          <Trash2 size={16} />
        </button>
      </div>
    </td>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-2xl w-full max-w-2xl border border-white/10 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <h3 className="text-xl font-semibold text-white">{product ? "Edit Product" : "Add Product"}</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={formData.sku} onChange={(e) => setFormData({ ...formData, sku: e.target.value })} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white" required />
            </div>
            <div>
              <label className="text-gray-400 text-xs mb-2 block">Barcode</label>
              <input type="text" value={formData.barcode || ""} onChange={(e) => setFormData({ ...formData, barcode: e.target.value })} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white" />
            </div>
          </div>
          <div>
            <label className="text-gray-400 text-xs mb-2 block">Product Name *</label>
            <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white" required />
          </div>
          <div>
            <label className="text-gray-400 text-xs mb-2 block">Description</label>
            <textarea value={formData.description || ""} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white resize-none" rows={3} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="relative">
              <label className="text-gray-400 text-xs mb-2 block">Category *</label>
              <button type="button" onClick={() => setShowCategoryDropdown(!showCategoryDropdown)} className="w-full flex items-center justify-between px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-left">
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-gray-900 border border-white/10 rounded-xl shadow-xl z-10 overflow-hidden">
                  <div className="p-2">
                    <div className="flex gap-2">
                      <input type="text" value={newCategory} onChange={(e) => setNewCategory(e.target.value)} placeholder="New category" className="flex-1 px-3 py-2 bg-white/10 border border-white/10 rounded-lg text-white text-sm" onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addCategory())} />
                      <button type="button" onClick={addCategory} className="px-3 py-2 bg-amber-500 text-white rounded-lg text-sm">Add</button>
                    </div>
                  </div>
                  <div className="max-h-40 overflow-y-auto">
                    {allCategories.map((cat) => (
                      <button key={cat} type="button" onClick={() => { setFormData({ ...formData, category: cat }); setShowCategoryDropdown(false); }} className="w-full px-4 py-2 text-left text-gray-300 hover:bg-white/5">{cat}</button>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div>
              ...
              <select value={formData.unit} onChange={(e) => setFormData({ ...formData, unit: e.target.value })} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" value={formData.cost_price} onChange={(e) => setFormData({ ...formData, cost_price: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white" min="0" step="0.01" required />
            </div>
            <div>
              <label className="text-gray-400 text-xs mb-2 block">Selling Price *</label>
              <input type="number" value={formData.selling_price} onChange={(e) => setFormData({ ...formData, selling_price: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white" min="0" step="0.01" required />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-gray-400 text-xs mb-2 block">Initial Stock</label>
              <input type="number" value={formData.stock_quantity} onChange={(e) => setFormData({ ...formData, stock_quantity: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white" min="0" step="0.01" />
            </div>
            <div>
              <label className="text-gray-400 text-xs mb-2 block">Reorder Level</label>
              <input type="number" value={formData.reorder_level} onChange={(e) => setFormData({ ...formData, reorder_level: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white" min="0" step="0.01" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-gray-400 text-xs mb-2 block">Tax Rate (%)</label>
              <input type="number" value={formData.tax_rate} onChange={(e) => setFormData({ ...formData, tax_rate: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white" min="0" max="100" step="0.01" />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-gray-400 text-xs mb-2 block">Options</label>
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={formData.is_taxable} onChange={(e) => setFormData({ ...formData, is_taxable: e.target.checked })} className="w-5 h-5 bg-white/10 border border-white/20 rounded" />
                <span className="text-gray-300 text-sm">Taxable</span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={formData.is_active} onChange={(e) => setFormData({ ...formData, is_active: e.target.checked })} className="w-5 h-5 bg-white/10 border border-white/20 rounded" />
                <span className="text-gray-300 text-sm">Active</span>
              </label>
            </div>
          </div>
          <div className="flex gap-3 pt-4">
            <button type="button" onClick={onClose} className="flex-1 py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-colors">Cancel</button>
            <button type="submit" disabled={isLoading} className="flex-1 py-3 bg-amber-500 hover:bg-amber-600 disabled:bg-gray-600 text-white font-medium rounded-xl transition-colors flex items-center justify-center gap-2">
              {isLoading ? <><Loader2 size={18} className="animate-spin" />Saving...</> : <><CheckCircle size={18} />{product ? "Update Product" : "Add Product"}</>}
            </button>
          </div>
        </form>
      </div>
  ...
  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div className="bg-gray-800 rounded-2xl p-6 max-w-md w-full border border-white/10">
      <div className="text-center">
        ...
        <p className="text-gray-400 mb-6">Are you sure you want to delete <span className="text-white font-medium">{product.name}</span>? This action cannot be undone.</p>
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-colors">Cancel</button>
          <button onClick={onConfirm} disabled={isLoading} className="flex-1 py-3 bg-red-500 hover:bg-red-600 disabled:bg-gray-600 text-white rounded-xl transition-colors flex items-center justify-center gap-2">
            {isLoading ? <Loader2 size={18} className="animate-spin" /> : <><Trash2 size={18} />Delete</>}
          </button>
        </div>
      </div>
    </div>
...
const ProductsPanel = ({
  ...
  const filtered = useMemo(() => {
    return products.filter((product) => {
      const matchesSearch = !search || product.name?.toLowerCase().includes(search.toLowerCase()) || product.sku?.toLowerCase().includes(search.toLowerCase()) || product.barcode?.includes(search);
      const matchesCategory = !categoryFilter || product.category === categoryFilter;
      const matchesLowStock = !showLowStock || (product.stock_quantity || 0) <= (product.reorder_level || 10);
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <p className="text-gray-400 text-sm">{filtered.length} product{filtered.length !== 1 ? "s" : ""}</p>
        <button onClick={onAdd} className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-xl transition-colors">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search products..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white" />
        </div>
        <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white">
        ...
        <label className="flex items-center gap-3 px-4 py-3 bg-white/5 border border-white/10 rounded-xl cursor-pointer">
          <input type="checkbox" checked={showLowStock} onChange={(e) => setShowLowStock(e.target.checked)} className="w-5 h-5 bg-white/10 border border-white/20 rounded" />
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={7} className="text-center py-12">
                ...
                <p className="text-gray-500 text-sm mt-1">{search || categoryFilter || showLowStock ? "Try adjusting your filters" : "Add your first product to get started"}</p>
              </td></tr>
            ) : (
...
const ReordersPanel = ({ refreshKey }: { refreshKey: number }) => {
  ...
  if (loading) return <div className="text-gray-400 text-sm">Loading reorder suggestions...</div>;

  return (
    <div>
      <div className="flex items-center gap-2 mb-4 text-amber-400 text-sm">
        <Bell size={16} />
        <span>Auto-generated reorder suggestions (updates in real-time as stock changes)</span>
      </div>
      {reorders.length === 0 ? (
        <div className="text-center py-12">
          <Package className="w-12 h-12 text-gray-600 mx-auto mb-3" />
          <p className="text-gray-400">No pending reorders</p>
          <p className="text-gray-500 text-sm mt-1">Suggestions appear automatically when stock drops below the reorder level</p>
        </div>
      ) : (
          ...
          {reorders.map((r: any) => (
            <div key={r.id} className="bg-white/5 border border-white/10 rounded-xl p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-400 text-xs">Current: {r.currentStock} · Reorder level: {r.reorderLevel} · Suggested: {r.suggestedQty} units</p>
              </div>
              <button onClick={() => handleFulfill(r.id, r.suggestedQty)} className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-sm flex items-center gap-2">
...
export default function InventoryManagement() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const cats = new Set(productsData.map((p: any) => p.category).filter(Boolean));
  ...
  const handleSaveProduct = async (formData: any) => {
    ...
    try {
      ...
      if (editingProduct) {
        ...
        emit({ type: "product:updated", productId: editingProduct.id, stationId: currentStation.id, data: formData, prev });
        showNotice("Product updated successfully");
      } else {
        const { data, error } = await supabase
          .from("products")
          .insert({ ...formData, station_id: currentStation.id, owner_id: ownerId })
          .select()
          .single();
        if (error) throw error;
        // Emit automation event — engine auto-records initial stock txn
        emit({ type: "product:created", productId: data.id, stationId: currentStation.id, data: formData });
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      ...
      if (error) throw error;
      emit({ type: "product:deleted", productId: deleteProduct.id, stationId: currentStation?.id || "" });
  ...
  const handleAdjustment = async (
    ...
    try {
      ...
      adjustments.forEach((a) =>
        emit({ type: "stock:adjusted", productId: a.productId, stationId: currentStation.id, newQty: a.newQuantity, reason: a.reason }),
      );
  ...
  const handleTransfer = async (data: {
    ...
    try {
      ...
      emit({ type: "stock:transfer", productId: data.productId, fromStationId: currentStation.id, toStationId: data.toStationId, qty: data.quantity });
  ...
  const handleCount = async (
    ...
    try {
      ...
      counts.filter((c) => c.variance !== 0).forEach((c) =>
        emit({ type: "stock:adjusted", productId: c.productId, stationId: currentStation.id, newQty: c.countedQuantity, reason: "Stock count adjustment" }),
      );
  ...
  const handleWastage = async (data: {
    ...
    try {
      ...
      emit({ type: "stock:wastage", productId: data.productId, stationId: currentStation.id, qty: data.quantity });
  ...
  return (
    <div className="p-6">
      ...
      <div className={activeTab === "products" ? "" : "max-w-2xl"}>
        ...
        {activeTab === "reorders" && <ReordersPanel refreshKey={reorderRefresh} />}
