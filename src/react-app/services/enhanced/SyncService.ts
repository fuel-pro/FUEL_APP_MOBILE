...
class EnhancedSyncService extends EventEmitter {
  ...
  public async pushChange(tableName: string, operation: 'INSERT' | 'UPDATE' | 'DELETE', data: any) {
    ...
    if (!this.isOnline) {
      ...
      this.updateSyncState(tableName, { pendingChanges: this.pendingQueue.length });
      return change.id;
    }
    ...
    try {
      ...
      this.updateSyncState(tableName, { 
        ...
        errors: [...this.syncStates.get(tableName)?.errors || [], (error as Error).message]
      });
    }
  ...
  private persistPendingChanges() {
    try {
      localStorage.setItem('fuelpro_pending_sync', JSON.stringify(this.pendingQueue));
    } catch (e) {
