  ...
  IN: {
    ...
    complianceFeatures: [
      ...
      },
      {
        ...
        name: "e-Invoicing",
        description:
          "IRN generation for B2B transactions (\u20B95Cr+ turnover)",
        required: true,
        category: "Tax",
      },
      {
...
function generateComplianceConfig(countryCode: string): ComplianceConfig {
  ...
  const paymentMethods: PaymentMethod[] = (paymentConfig?.paymentMethods || [])
    ...
    .map((m) => ({
      ...
      chargeRate:
        m.type === "card" ? 0.015 : m.type === "digital_wallet" ? 0.02 : 0,
    }));
  ...
  return {
    ...
    vatName:
      detailed?.vatName || (isEU ? "VAT" : vatRate > 0 ? "VAT/GST" : "Tax"),
    hasETR:
      detailed?.hasETR ??
      (isEU ||
    ...
    etrName:
      detailed?.etrName ||
      (isEU ? "VAT Compliant POS" : "Electronic Receipt System"),
    etrFormat: detailed?.etrFormat || `ETR-{station}-{date}`,
    fuelRegulator:
      detailed?.fuelRegulator || `${countryName} Energy/Fuel Authority`,
    ...
    receiptRequirements:
      detailed?.receiptRequirements || DEFAULT_RECEIPT_REQUIREMENTS,
    ...
    bankSupport: detailed?.bankSupport || [
      ...
      {
        code: "bank2",
        name: `${countryName} Commercial Bank`,
        supportsApi: false,
        supportsStatementImport: true,
      },
    ],
    ...
    isEU,
    dataProtectionLaw:
      detailed?.dataProtectionLaw ||
      (isEU ? "EU GDPR" : `${countryName} Data Protection Law`),
  };
}
...
export function getAllComplianceCountries(): {
  key: string;
  code: string;
  name: string;
}[] {
