...
// ⚠️ SECURITY NOTE: R2 configuration should ONLY contain public values.
// Secret keys must NEVER be exposed to client-side code.
// Uploads should use pre-signed URLs generated by server-side API functions.
const R2_CONFIG = {
  accountId: import.meta.env.VITE_CLOUDFLARE_ACCOUNT_ID || "",
  // These are PUBLIC identifiers only - NOT secrets
  ...
  // ⚠️ IMPORTANT: accessKeyId and secretAccessKey have been REMOVED from client-side code.
  // To upload files securely, use the /api/r2/upload-url endpoint which generates
  // pre-signed URLs server-side where secrets are safely stored.
};

// ⚠️ SECURITY NOTE: Upstash Redis token must NEVER be exposed client-side.
// All Redis operations should go through serverless API functions.
const UPSTASH_CONFIG = {
  url: import.meta.env.VITE_UPSTASH_REDIS_REST_URL || "",
  // ⚠️ IMPORTANT: token has been REMOVED from client-side code.
  // Use /api/cache/* endpoints instead, which handle authentication server-side.
};
...
export const R2Storage = {
  async uploadFile(file: File | Blob, path: string): Promise<string | null> {
    // ⚠️ SECURITY: Direct client-side R2 uploads removed.
    // Use server-side API to generate pre-signed URLs instead.
    // Fallback to Supabase Storage which handles auth properly.
    console.warn("Direct R2 uploads disabled for security. Using Supabase Storage.");
...
// ─── Upstash Redis (Sessions & Cache) ───
// ⚠️ SECURITY: Direct client-side Upstash access disabled.
// All Redis operations must go through serverless API functions (/api/cache/*)
// to protect the authentication token from exposure.
export const UpstashCache = {
  async get<T>(key: string): Promise<T | null> {
    // Use server-side API endpoint instead of direct Upstash access
    try {
  ...
  async del(key: string): Promise<boolean> {
    // Use server-side API endpoint instead of direct Upstash access
    try {
      const res = await fetch("/api/cache/del", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ key }),
      });
