...
export function usePerformanceMonitor(callback?: (metrics: PerformanceMetrics) => void) {
  const metricsRef = useRef<PerformanceMetrics>({ timestamp: new Date().toISOString() });
...
export function scheduleIdleTask(callback: (deadline: IdleDeadline) => void, timeout?: number) {
