...
import { Package, TrendingUp, AlertTriangle, CheckCircle, Clock, Zap, Brain, RefreshCw } from 'lucide-react';
...
const EnhancedInventoryManagement: React.FC = () => {
  ...
  const generateDemandForecasts = (items: InventoryItem[]): DemandForecast[] => {
    return items.map(item => {
      ...
      const predictedDemand = Math.round(avgDailySales * 30 * seasonalityFactor);
      const daysUntilStockout = item.current_stock > 0 
  ...
  const generateAutomatedOrders = useCallback((items: InventoryItem[], forecasts: DemandForecast[]) => {
    ...
    items.forEach((item, index) => {
      ...
      if (item.current_stock < item.min_stock || forecast.daysUntilStockout < 7) {
        newOrders.push({
  ...
  const inventoryMetrics = useMemo(() => {
    ...
    const lowStockItems = inventory.filter(i => i.current_stock < i.min_stock).length;
    const outOfStockItems = inventory.filter(i => i.current_stock === 0).length;
    const overStockItems = inventory.filter(i => i.current_stock > i.max_stock).length;
    const totalValue = inventory.reduce((sum, i) => sum + (i.current_stock * i.unit_price), 0);
    ...
    return {
      ...
      overStockItems,
      totalValue,
      healthScore: Math.round(((totalItems - lowStockItems - outOfStockItems) / totalItems) * 100) || 0,
    };
  }, [inventory]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{inventoryMetrics.overStockItems}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{inventoryMetrics.healthScore}%</div>
                ...
                return (
                  <div key={idx} className="bg-white dark:bg-slate-800 p-4 rounded-lg">
                    ...
                    <div className="text-sm text-slate-600 dark:text-slate-400 space-y-1">
                      <p>Predicted Demand: <span className="font-semibold">{forecast.predictedDemand} units/month</span></p>
                      <p>Confidence: <span className="font-semibold">{forecast.confidence}%</span></p>
                      <p>Days to Stockout: <span className={`font-semibold ${forecast.daysUntilStockout < 7 ? 'text-red-600' : 'text-green-600'}`}>
                    ...
                    <Button
                      ...
                      onClick={() => openOrderDialog(item!, forecast.recommendedOrderQty)}
                    >
                  ...
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        {stockStatus === 'low_stock' && <AlertTriangle className="w-4 h-4 text-amber-500" />}
                        {stockStatus === 'out_of_stock' && <AlertTriangle className="w-4 h-4 text-red-500" />}
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge
                        variant={
                          stockStatus === 'out_of_stock' ? 'destructive' :
                          stockStatus === 'low_stock' ? 'warning' :
                          stockStatus === 'over_stock' ? 'secondary' : 'default'
                        }
                      >
        ...
        <DialogContent>
          ...
          {selectedItem && (
            <div className="space-y-4 py-4">
              <div>
                <Label>Product</Label>
                <div className="font-medium mt-1">{selectedItem.product_name}</div>
              </div>

              <div>
                <Label>Current Stock</Label>
                <div className="font-medium mt-1">{selectedItem.current_stock} units</div>
              </div>
