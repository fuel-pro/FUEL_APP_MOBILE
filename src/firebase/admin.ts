...
const firebaseAdminConfig: admin.ServiceAccount = {
  ...
  clientEmail: process.env.FIREBASE_CLIENT_EMAIL || 'firebase-adminsdk-fbsvc@fuel-pro-1.iam.gserviceaccount.com',
};
  ...
  try {
    ...
    if (firebaseAdminConfig.projectId && firebaseAdminConfig.privateKey && firebaseAdminConfig.clientEmail) {
      adminApp = admin.initializeApp({
        ...
        databaseURL: process.env.FIREBASE_DATABASE_URL || 'https://fuel-pro-1.firebaseio.com',
      });
    } else {
      // Fallback: Initialize without credentials for development
      console.warn('[Firebase Admin] Missing credentials, initializing with default config');
      adminApp = admin.initializeApp({
...
export async function verifySessionCookie(sessionCookie: string): Promise<admin.auth.UserRecord> {
  try {
    const auth = getAdminAuth();
