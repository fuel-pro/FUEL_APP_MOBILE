...
// Apply authentication to all routes (FIXED: Was completely open)
router.use(protect);

// Founder-only routes for sensitive operations
const founderOnly = ['secrets', 'users', 'config'];
router.use('/data/:collection', (req, res, next) => {
  const { collection } = req.params;
  if (founderOnly.includes(collection) && req.user.role !== 'founder') {
...
router.post('/seed', authorize('founder'), (req, res) => {
  const seedData = {
    feature_flags: {
      pos_system: { name: 'POS System', enabled: true },
      mpesa_live: { name: 'M-PESA Live', enabled: true },
      ai_chatbot: { name: 'AI Chatbot', enabled: true }
    }
  };
  
  for (const [collection, data] of Object.entries(seedData)) {
    saveCloudRecord(collection, collection, data);
  }
