import{h as z,F as T,a2 as H,T as S,a5 as B}from"./index-B-T48S1p.js";import{j as e}from"./trpc-Do2GXDyq.js";import{r as u}from"./vendor-DwGTNw7s.js";import{E as $}from"./jspdf.es.min-CZheozpm.js";import O from"./html2canvas.esm-QH1iLAAe.js";import{P as A}from"./printer-DkGcASSq.js";function q(){const{state:a}=z(),[i,P]=u.useState(new Date().getMonth()+1),[c,C]=u.useState(new Date().getFullYear()),[N,R]=u.useState([]),[D,F]=u.useState(!1),[x,M]=u.useState({petrol:0,diesel:0,total:0}),y=["January","February","March","April","May","June","July","August","September","October","November","December"],Y=(()=>{const t=new Date().getFullYear(),o=t-100,d=t+100,s=[];for(let r=o;r<=d;r++)s.push(r);return s})();u.useEffect(()=>{E()},[i,c,a.salesHistory]);const E=()=>{const t=[];a.salesHistory&&typeof a.salesHistory=="object"&&Object.entries(a.salesHistory).forEach(([s,r])=>{const[m]=s.split("_"),l=new Date(m);if(!isNaN(l.getTime())&&l.getMonth()+1===i&&l.getFullYear()===c&&r&&(r.pmsPumps||r.agoPumps)){const p=Math.round((r.pmsPumps||[]).reduce((v,w)=>v+(w.salesKsh||0),0)*100)/100,h=Math.round((r.agoPumps||[]).reduce((v,w)=>v+(w.salesKsh||0),0)*100)/100,g=l.getDate().toString().padStart(2,"0"),b=i.toString().padStart(2,"0"),f=c.toString(),j=r.shift==="Night"||r.shift==="NIGHT"?"NIGHT":"DAY",I=`${g}/${b}/${f}(${j})`;(p>0||h>0)&&t.push({date:I,shift:j,petrolSales:p,dieselSales:h,totalSales:Math.round((p+h)*100)/100})}}),t.sort((s,r)=>{const m=new Date(s.date.split("(")[0].split("/").reverse().join("-")),l=new Date(r.date.split("(")[0].split("/").reverse().join("-"));return m.getTime()-l.getTime()}),R(t);const o=t.reduce((s,r)=>s+r.petrolSales,0),d=t.reduce((s,r)=>s+r.dieselSales,0);M({petrol:o,diesel:d,total:o+d})},k=()=>{const t=document.getElementById("report-content");if(!t)return;const o=window.open("","_blank");o&&(o.document.write(`
      <html>
        <head>
